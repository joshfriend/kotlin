...
/**
 * Resolves an external field reference (e.g., Kotlin const val) to its constant value.
 * Used for cross-language constant evaluation in Java field initializers.
 *
 * Handles several cases:
 * 1. Top-level Kotlin properties: `MainKt.FOO` where `FOO` is `const val FOO = 1` in Main.kt
 *    - `MainKt` is a facade class generated by JVM backend, but in FIR it's a top-level property
 * 2. Companion object const vals: `Foo.BAR` where `BAR` is in `Foo.Companion`
 * 3. Regular class member properties
 *
 * @param session the FIR session
 * @param classQualifier the qualifier (class name) or null for unqualified references
 * @param fieldName the field/property name
 * @param currentPackage the package of the current Java class (for resolving unqualified names)
 * @return the constant value, or null if resolution fails
 */
private fun resolveExternalFieldValue(
    ...
    if (classQualifier == null) {
        // Unqualified reference - not supported for cross-language
        return null
    }

    val propertyName = Name.identifier(fieldName)

    // Case 1: Check if this is a reference to a top-level Kotlin property via its facade class
    ...
    val classIds = if (parts.size == 1) {
        // Simple class name - try current package first, then top-level
        listOf(
