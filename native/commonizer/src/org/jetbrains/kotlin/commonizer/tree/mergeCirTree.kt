...
internal fun CirNodeWithMembers<*, *>.buildClass(
    ...
    val classNode = classes.getOrPut(treeClass.clazz.name) {
        buildClassNode(
            ...
            ParentNode(parent),
            treeClass.id,
            context.supportExpectClassSupplier,
        )
    }
...
internal fun CirNodeWithMembers<*, *>.buildFunction(
    ...
    val functionNode = functions.getOrPut(
        ...
        buildFunctionNode(context.storageManager, context.targets, context.classifiers, context.commonizerSettings, ParentNode(parent), context.supportExpectClassSupplier)
    }
...
internal fun CirNodeWithMembers<*, *>.buildProperty(
    ...
    val propertyNode = properties.getOrPut(
        ...
        buildPropertyNode(context.storageManager, context.targets, context.classifiers, context.commonizerSettings, ParentNode(parent), context.supportExpectClassSupplier)
    }
    /* Multiple type substitutions could in result in the same commonization result */
...
internal fun CirPackageNode.buildTypeAlias(context: TargetBuildingContext, treeTypeAlias: CirTreeTypeAlias) {
    val typeAliasNode = typeAliases.getOrPut(treeTypeAlias.typeAlias.name) {
        buildTypeAliasNode(context.storageManager, context.targets, context.classifiers, context.commonizerSettings, treeTypeAlias.id, context.supportExpectClassSupplier)
    }
