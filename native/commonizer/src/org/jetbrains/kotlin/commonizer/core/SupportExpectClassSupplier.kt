...
class SupportExpectClassSupplier internal constructor(
    ...
    private val supportLibraryRepository: Repository,
) {
    private val modulesProviderCache = mutableMapOf<CommonizerTarget, ModulesProvider>()

    private fun supportLibraryModulesProviderFor(target: CommonizerTarget) =
        modulesProviderCache.getOrPut(target) { buildModulesProvider(supportLibraryRepository, target) }
    ...
    private fun findSupportExpectClassFor(targetsToTypes: Map<CommonizerTarget, CirClassType>): CirEntityId? {
        ...
        val targetsWithTheirDependencies = flattenedTargetsToTypes.mapValues { (target, _) -> getProvidedClassifiers(target) }
        val commonSupportClassifiers = loadAllLeafClassifiersAvailableFor(SharedCommonizerTarget(targetsToTypes.keys.allLeaves()))

        return commonSupportClassifiers.entries.find { (cirEntityId, _) ->
