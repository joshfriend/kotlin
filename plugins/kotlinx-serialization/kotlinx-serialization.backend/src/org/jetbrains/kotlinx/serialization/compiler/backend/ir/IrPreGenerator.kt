...
class IrPreGenerator(
    ...
    private fun preGenerateWriteSelfMethodIfNeeded() {
        ...
        if (!irClass.shouldHaveSpecificSyntheticMethods(isJvm = true) {
    ...
    private fun preGenerateDeserializationConstructorIfNeeded() {
        ...
        if (irClass.isSingleFieldValueClass(distinguishBasicAndExtended = compilerContext.platform.isJvm())) return
