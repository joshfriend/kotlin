...
sealed interface SpecializedTypeAbi {
    ...
    companion object {
        fun fromLightIrType(type: LightIrType): SpecializedTypeAbi? {
            ...
            // Non-nullable inline class
            if (!type.nullable && classifier.inlineJvmRepr != null) {
                return InlineClass(
                    classifier.fqName.replace('.', '/'),
                    classifier.inlineJvmRepr,
                )
            }

            // Nullable inline class
            if (type.nullable && classifier.nullableInlineJvmRepr != null) {
                return InlineClass(
                    classifier.fqName.replace('.', '/'),
                    classifier.nullableInlineJvmRepr,
                )
            }
...
data class InlineClass(override val boxedInternalName: String, override val reprDesc: String) : SpecializedTypeAbi {
