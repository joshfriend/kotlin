...
internal class MetadataArgumentsImpl(
  ...
  override fun deepCopy(): MetadataArgumentsImpl = MetadataArgumentsImpl(adapter, restrictedArgViolations.toList()).also { newArgs -> newArgs.applyCompilerArguments(toCompilerArguments()) }
  ...
  public fun applyCompilerArguments(arguments: K2MetadataCompilerArguments) {
    ...
    try { this[X_TARGET_PLATFORM] = arguments.targetPlatform.map { MetadataTargetPlatform.entries.firstOrNull { entry -> entry.stringValue == it } ?: throw CompilerArgumentsParseException("Unknown -Xtarget-platform value: $it") }.toTypedArray() } catch (_: NoSuchMethodError) {  }
